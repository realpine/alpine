...
static char rcsid[] = "$Id: help.c 245 2006-11-18 02:46:41Z hubert@u.washington.edu $";
#endif
 ...
 * Internal prototypes
 */
int	 helper_internal(HelpType, char **, char *, char *, int);
...
the first line in the text.
  -----*/
int
helper_internal(HelpType text, char **otext, char *frag, char *title, int flags)
{
    char	  **shown_text;
    int		    cmd = MC_NONE;
    long	    offset = 0L;
...
{
    snprintf(buf, buflen, "  Pine Help%s%.*s%*s%d",
	    hprt->title_len ? ": " : " Text",
...
url_local_helper(char *url)
{
	...
	if(newhelp != NO_HELP){
	    ...
	    rv = helper_internal(newhelp, NULL, frag, _("HELP SUB-SECTION"),
...
url_local_config(char *url)
{
	...
	if(config){
	    ...
	    rv = helper_internal(NO_HELP, config, NULL, _("HELP CONFIG"),
				 HLPD_NEWWIN | HLPD_SIMPLE | HLPD_FROMHELP);
    ...
    HelpType section;
{
    ...
    if ((help_lines = get_help_text (section)) != NULL){
