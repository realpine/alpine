...
void	   post_compose_filters(BODY *);
	...
	PINEFIELD **custom, ACTION_S **role, int flags)
{
    ...
    if(so = (void *) so_get(PicoText, NULL, EDIT_ACCESS))
		...
		if(build_address(*tobufp, &addr, &errbuf,
			...
			if(outgoing->to || outgoing->cc || outgoing->bcc){
			    ...
			    result = call_mailer(header, *body, alt_smtp,
						 call_mailer_flags, call_mailer_file_result);
		...
		switch(set_default_hdrval(pf, custom)){
		  ...
		  case Combine:
			...
			if(pf->textbuf && *pf->textbuf){
			    ...
			    combined_hdr[l] = '\0';
			    strncat(combined_hdr, ",", l-strlen(combined_hdr));
			    combined_hdr[l] = '\0';
			    strncat(combined_hdr, pf->textbuf, l-strlen(combined_hdr));
			    combined_hdr[l] = '\0';
	   ...
	   _("Composition saved to Form Letter Folder. Select Compose to send."));
		...
		if((ps_global->post->pid = fork()) == 0){
			...
			if(role && role->smtp){
		        ...
		        result |= (call_mailer(&header, *body, alt_smtp,
					       call_mailer_flags, call_mailer_file_result) > 0)
...
message_format_for_pico(long int n, int (*f) (int))
{
    ...
    if(!format_message(mn_m2raw(ps_global->msgmap, n), e, b, NULL,
		       FM_NEW_MESS | FM_DISPLAY | FM_NOCOLOR | FM_NOINDENT | FM_UTF8, f)){
	q_status_message(SM_ORDER|SM_DING,3,3,"Error inserting Message");
	flush_status_messages(0);
	rv = 0L;
    }
...
send_exit_for_pico(struct headerentry *he, void (*redraw_pico)(void), int allow_flowed)
{
	...
	*p = c;
	...
	if(double_rad +
	   (call_mailer_flags ? 4 : F_ON(F_DSN, ps_global) ? 1 : 0) > 11)
	  ...
	  rv = radio_buttons(tmp_20k_buf, -FOOTER_ROWS(ps_global), opts,
			   'y', 'z',
			   (double_rad +
			    (call_mailer_flags ? 4 : F_ON(F_DSN, ps_global)
			     ? 1 : 0) == 11)
				   ? NO_HELP : 
			   (F_ON(F_DSN, ps_global) && allow_flowed)
					          ? h_send_prompt_dsn_flowed :
 ...
 */
void
post_compose_filters(struct mail_bodystruct *body)
{
}
...
/*----------------------------------------------------------------------
 ...
 -----*/
void
outgoing2strings(METAENV *header, struct mail_bodystruct *bod, void **text, PATMT **pico_a)
{
    PINEFIELD  *pf;
 ...
 ****/
	...
	if(set_mime_type_by_extension(&p->body, pa->filename))
	  set_only_charset_by_grope(&p->body, ps_global->keyboard_charmap);
	else
	  set_mime_type_by_grope(&p->body, ps_global->keyboard_charmap);
	...
	*parmp		    = mail_newbody_parameter();
	...
	if(pa->filename)
	  (*parmp)->value = cpystr((lc = last_cmpnt(pa->filename)) ? lc : pa->filename);
...
void
set_mime_flowed_text(struct mail_bodystruct *body)
{
    PARAMETER *pm;
...
build_address(char *to, char **full_to, char **error, BUILDER_ARG *barg, int *mangled)
{
	...
	if(p == q || (pt && pt->encoded && pt->encoded->etext)){
		...
		if(len >= pt->encoded->cksumlen){
		     ...
		     * stuff as it is now.
		     */
		    if(len == pt->encoded->cksumlen+2 && p[0] == '"' && strchr(p+1, '"')){
			char *tmp, *quote, *r;
			...
			*quote = '\0';
			...
			r = tmp;
			sstrncpy(&r, p+1, len-1);
			sstrncpy(&r, quote+1, len-1-(r-tmp));
			tmp[len-1-1] = '\0';
			*quote = '"';
		...
		if(!pt || !pt->encoded ||
		  ...
		  if(csum_mid != pt->encoded->cksumval){ /* else, leave alone */
		    ...
		    l = strlen(etext1) + strlen(etext2) + strlen(etext3);
		    pt->encoded->etext = (char *) fs_get((l+1) * sizeof(char));

		    r = pt->encoded->etext;
		    sstrncpy(&r, etext1, l+1);
		    sstrncpy(&r, etext2, l+1-(r-pt->encoded->etext));
		    sstrncpy(&r, etext3, l+1-(r-pt->encoded->etext));
		    pt->encoded->etext[l] = '\0';
