...
cgi_html {
  cgi_head {
    ...
    if {$view} {
      set normalreload [cgi_buffer {WPHtmlHdrReload "[file join $_wp(appdir) wp.tcl?page=addrbook]"}]
	...
	cgi_script  type="text/javascript" language="JavaScript" {
	  ...
	  cgi_put  " reloadtimer = window.setInterval('wpLink(); window.location.replace(\\'[cgi_root]/$_wp(appdir)/wp.tcl?page=addrbook&reload=1\\')', t * 1000);"
	  cgi_puts "}"
	}
  ...
  cgi_body bgcolor=$_wp(bordercolor) background=[file join $_wp(imagepath) logo $_wp(logodir) back.gif] "style=\"background-repeat: repeat-x\"" $onload $onunload {
    ...
    cgi_table border=0 cellspacing=0 cellpadding=0 width="100%" height="100%" {
      if {$browse} {
	cgi_puts "<form action=\"[cgi_root]/$_wp(appdir)/wp.tcl\" name=addrchoice method=get target=_top>"
	...
	cgi_table_data valign=top width=100% class=dialog {
	  ...
	  if {[info exists directories]} {
		...
		cgi_form $_wp(appdir)/wp method=post enctype=multipart/form-data name=ldapsearch${i} {
