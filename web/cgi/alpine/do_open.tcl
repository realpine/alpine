...
if {[catch {WPLoadCGIVar cancel}] == 0 && [string compare Cancel $cancel] == 0} {
  ...
  cgi_http_head {
    cgi_redirect [cgi_root]/$_wp(appdir)/wp.tcl?page=${oncancel}.tcl
  }
} else {
  ...
  if {[info exists src] == 0 && [catch {eval WPCmd PEMailbox open $path} reason]} {
	...
	if {[regexp {^([a-zA-Z\.]+).*\/user=([^ /]*)} [lindex $coln 4] dummy srvname authuser]} {
      ...
      WPCmd set authpage [WPPercentQuote "[cgi_root]/$_wp(appdir)/open.tcl?folder=${folder}&colid=${colid}"]
      WPCmd set authcancel [WPPercentQuote "[cgi_root]/$_wp(appdir)/wp.tcl?page=folders"]

      set src [file join $_wp(cgipath) $_wp(appdir) fr_queryauth.tcl]
