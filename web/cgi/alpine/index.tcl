...
if {$queryexpunge == 1 || [string compare [string tolower $queryexpunge] expunge] == 0} {
  ...
  if {[WPCmd PEMailbox selected] == 0} {
    ...
    set uid 0
    source [file join $_wp(cgipath) $_wp(appdir) fr_promptsave.tcl]
    set nopage 1
  }
	...
	switch -exact -- $f_name {
	  ...
	  default {
		...
		if {[catch {WPCmd PEInfo set querycreate_state $qstate}] == 0} {
		  source [file join $_wp(cgipath) $_wp(appdir) fr_querycreate.tcl]
		  set nopage 1
		} else {
  ...
  switch -- $selectop {
    ...
    text {
      set uid 0
      source [file join $_wp(cgipath) $_wp(appdir) fr_seltext.tcl]
      set nopage 1
    }
    date {
      set uid 0
      source [file join $_wp(cgipath) $_wp(appdir) fr_seldate.tcl]
      set nopage 1
    }
    stat {
      set uid 0
      source [file join $_wp(cgipath) $_wp(appdir) fr_selstat.tcl]
      set nopage 1
    }
    zoom {
      if {[WPCmd PEMailbox selected] == 0} {
    ...
    default {
      set uid 0
      source [file join $_wp(cgipath) $_wp(appdir) fr_select.tcl]
      set nopage 1
    }
  }
} elseif {$zoom != ""} {
  ...
  source [file join $_wp(cgipath) $_wp(appdir) folders.tcl]
  set nopage 1
...
if {![info exists nopage]} {
  ...
  cgi_html {
	...
	set onsubmit ""
      ...
      set normalreload [cgi_buffer {WPHtmlHdrReload "$_wp(appdir)/wp.tcl?page=index"}]
      if {[info exists _wp(exitonclose)]} {
	cgi_script  type="text/javascript" language="JavaScript" {
	  cgi_put  "function indexReloadTimer(t){"
	  cgi_put  " reloadtimer = window.setInterval('wpLink(); window.location.replace(\\'[cgi_root]/$_wp(appdir)/wp.tcl?page=index&reload=1\\')', t * 1000);"
	  cgi_puts "}"
	}
	...
	set enctype "application/x-www-form-urlencoded"
      ...
      cgi_form $_wp(appdir)/wp method=$postmethod enctype=$enctype name=index $onsubmit target=body {
